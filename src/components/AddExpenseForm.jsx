...
const AddExpenseForm = ({ onAddExpense, categories }) => {
    const [expense, setExpense] = useState({ // Renamed 'income' state to 'expense' for clarity
        ...
        amount: "",
        date: "",
        iconUrl: "", // Icon might be associated with the selected category, or kept separate for custom entries
    });
    ...
    const categoryOptions = categories.map((cat) => ({
        ...
        label: `${cat.categoryName}`, // Display icon and name in dropdown
    }));

    return (
        <div>
            <EmojiPickerPopup
                iconUrl={expense.iconUrl} // Uses expense.icon now
